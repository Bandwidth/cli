...
func (s *Service) get(path string) (*Envelope, error) {
...
func (s *Service) ListBrands(limit, offset int, filters []Filter) (*Envelope, error) {
	return s.get(s.base() + "/brands" + EncodeQuery(limit, offset, filters))
}

// GetBrand returns one brand. The response wraps data as an object, not a
// single-element array — verified against production.
func (s *Service) GetBrand(brandID string) (*Envelope, error) {
	if brandID == "" {
...
func (s *Service) ListCampaigns(limit, offset int, filters []Filter) (*Envelope, error) {
	return s.get(s.base() + "/campaigns" + EncodeQuery(limit, offset, filters))
}

// GetCampaign returns one campaign, including the fields the list omits.
func (s *Service) GetCampaign(campaignID string) (*Envelope, error) {
	if campaignID == "" {
